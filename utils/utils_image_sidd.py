...
def get_image_paths_sidd(dataroot):
    paths = None  # return None if dataroot is None
    clean_imgs = []
    noise_imgs = []
    if dataroot is not None:
        ...
        for ind, folder in enumerate(folders):
            clean_imgs.extend(glob.glob(folder + '/*GT_SRGB*'))
            noise_imgs.extend(glob.glob(folder + '/*NOISY_SRGB*'))
        # path = {
        #     'paths_H': clean_imgs,
        #     'paths_L': noise_imgs
        # }
        ...
        # print('ready')
        return clean_imgs, noise_imgs
    return paths
def get_image_paths_sidd_mirnet(dataroot):
    if dataroot is not None:
        clean_files = sorted(os.listdir(os.path.join(dataroot, 'groundtruth')))
        noisy_files = sorted(os.listdir(os.path.join(dataroot, 'input')))

        clean_filenames = [os.path.join(dataroot, 'groundtruth', x) for x in clean_files]
        noisy_filenames = [os.path.join(dataroot, 'input', x) for x in noisy_files]

    return clean_filenames, noisy_filenames
...
# --------------------------------------------
def imread_uint(path, n_channels=3, train_transformer=None):
    #  input: path
    ...
    if n_channels == 1:
        ...
        if train_transformer:
            ...
            img = train_transformer(PIL_image)
            img = np.array(img)
            # print(img)
            # print('trans----------------------')
